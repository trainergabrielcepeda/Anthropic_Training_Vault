...
function mockExecute(name: string, toolInput: unknown): string {
  return JSON.stringify({ tool: name, received: toolInput });
}

async function step1ForcedToolChoice(): Promise<{ response: Anthropic.Message; block: ToolUseBlock }> {
  ...
  if (!block) throw new Error("Expected a forced tool_use block");
  console.log(`  Forced call: ${block.name}(${JSON.stringify(block.input)})`);
  return { response, block };
}

async function step2AutoFollowup(firstResponse: Anthropic.Message, metadataBlock: ToolUseBlock): Promise<void> {
...
async function step3AnyGuaranteesToolCall(): Promise<void> {
