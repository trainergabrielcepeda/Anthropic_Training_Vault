...
const DIFFERENTIATED_TOOLS: Tool[] = [
  {
...
async function showRouting(tools: Tool[], label: string): Promise<void> {
  console.log(`\n=== ${label} ===`);
  for (const query of TEST_QUERIES) {
    ...
    console.log(`    -> routed to: ${toolUse && toolUse.type === "tool_use" ? toolUse.name : "NONE"}`);
  }
}
