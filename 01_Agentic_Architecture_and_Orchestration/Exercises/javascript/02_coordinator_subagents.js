 ...
 *   doc-analysis) would instead be an `AgentDefinition` (description +
 ...
 */
async function callSubagent(role, systemPrompt, userPrompt) {
...
async function dispatchSubagentsInParallel(jobs) {
  return Promise.all(jobs.map((j) => callSubagent(j.role, j.systemPrompt, j.userPrompt)));
}

function coordinatorDecompose(goal) {
  return [
    {
      role: 'web-researcher',
      systemPrompt:
...
async function coordinatorSynthesize(goal, subagentResults) {
  const findingsBlock = subagentResults
    .map((r) => `### ${r.role} findings\n${r.output}`)
    .join('\n\n');
 ...
 * narrow (Task 1.2). */
async function coordinatorCheckCoverageGaps(goal, synthesis) {
  const response = await client.messages.create({
    model: COORDINATOR_MODEL,
    max_tokens: 256,
    system:
  ...
  const text = (response.content.find((b) => b.type === 'text')?.text ?? 'NONE').trim();
  return text.toUpperCase().startsWith('NONE') ? null : text;
}

async function runCoordinator(goal) {
